...
example {a b : ℚ} (h1 : a - 3 = 2 * b) : a ^ 2 - a + 3 = 4 * b ^ 2 + 10 * b + 9 :=
  calc
    a^2 - a + 3
      = a^2 - (a - 3) := by ring
    _ = a^2 - 2*b := by rw [h1]
    _ = (a-3)^2 + 6*a - 9 - 2*b := by ring
    _ = (2*b)^2 + 6*a - 9 - 2*b := by rw [h1]
    _ = (2*b)^2 + 6*(a - 3) + 9 - 2*b := by ring
    _ = (2*b)^2 + 6*(2*b) + 9 - 2*b := by rw [h1]
...
example {z : ℝ} (h1 : z ^ 2 - 2 = 0) : z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = 3 :=
  calc
    z^4 - z^3 - z^2 + 2*z + 1
      = z^4 - z^3 + 2*z - (z^2 - 2) - 1 := by ring
    _ = z^4 - z^3 + 2*z - 0 - 1 := by rw [h1]
    _ = z^4 - z^3 + 2*z - 1 := by ring
    _ = z^4 - z*(z^2 - 2) - 1 := by ring
    _ = z^4 - z*0 - 1 := by rw [h1]
    _ = z^4 - 1 := by ring
    _ = (z^2 - 2)*(z^2 + 2) + 3 := by ring
    _ = 0*(z^2 + 2) + 3 := by rw [h1]
    _ = 3 := by ring
...
example {t : ℚ} (ht : t ^ 2 - 4 = 0) :
  ...
  calc
    ...
    _ = t^4 + 3*t*(t^2 - 4 + 10/3) - 3*t^2 - 2 := by ring
    _ = t^4 + 3*t*(0 + 10/3) - 3*t^2 - 2 := by rw [ht]
    _ = t^4 + 10*t - 3*t^2 - 2 := by ring
    _ = t^2*(t^2 - 4 + 1) + 10*t - 2 := by ring
    _ = t^2*(0 + 1) + 10*t - 2 := by rw [ht]
    _ = t^2 - 4 + 10*t + 2 := by ring
    _ = 0 + 10*t + 2 := by rw [ht]
    _ = 10*t + 2 := by ring
...
example {x y : ℝ} (h1 : x + 3 = 5) (h2 : 2 * x - y * x = 0) : y = 2 :=
  calc
    y = 2 - (2*x - y*x)/2 + ((x + 3) - 5)*(1 - y/2) := by ring
    _ = 2 - 0/2 + (5 - 5)*(1 - y/2) := by rw [h1, h2]
    _ = 2 := by ring


-- Es ist unmöglich, dass das Resultat -4 ergibt, trotzdem ist das ein gültiger Beweis.
-- Mir ist nicht ganz klar warum das kein Problem ist.
